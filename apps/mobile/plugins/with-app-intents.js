 ...
 *
 * Why a config plugin?
 *   Same CNG constraint as with-store-kit-configuration: ios/ is regenerated
 *   on every `expo prebuild`, so the Swift source must be (re)wired into the
 ...
 *
 * What it does:
 *   1. Copies plugins/app-intents/PegadaAppIntents.swift into the generated
...
const SOURCE_DIR = path.join(__dirname, "app-intents");
...
const withAppIntents = (config) => {
  return withXcodeProject(config, (modConfig) => {
    const { projectRoot, platformProjectRoot: platformRoot } =
    ...
    // with-store-kit-configuration resource wiring.
    fs.copyFileSync(
    ...
    // The "//" key is the file's JSON "comment", not a locale.
    const locales = Object.entries(phrasesByLocale).filter(
      ([lang]) => lang !== "//",
    );

    for (const [lang, phrases] of locales) {
      ...
      const alreadyAdded = group?.children?.some(
        ({ comment }) => comment === STRINGS_FILE_NAME,
      );
