...
Backs PR "perf(web): serve the hero through next/image and right-size the icons"
(branch `web/hero-delivery`, pegada-web wave 1, PR 3/4).
...
| --- | --- | --- | --- |
| hero mockup | `public/phone-mockup.png`, 3924x7620, 4,924,610 B (4,809 KB) | `src/assets/phone-mockup.webp`, 1308x2540, 160,498 B (157 KB) | -96.7% |
| favicon | `public/favicon.ico`, one 245x256 frame, 259,134 B (253 KB) | `public/favicon.ico`, 16/32/48 frames, 15,086 B (14.7 KB) | -94.2% |
| icon set (new files) | — | favicon-16x16 535 B, favicon-32x32 1,195 B, icon-192 5,951 B, icon-512 15,266 B, apple-touch-icon 3,577 B | +26,524 B |
...
| --- | --- | --- | --- | --- | --- | --- |
| desktop 1440x900 | 1 | w=640 | 4,924,610 | 61,270 | 4,962,236 | 99,329 |
| desktop 1440x900 | 2 | w=828 | 4,924,610 | 86,088 | 4,962,236 | 124,147 |
| mobile 390x844 | 1 | w=384 | 4,924,610 | 31,658 | 4,962,236 | 69,717 |
| mobile 390x844 | 2 | w=828 | 4,924,610 | 86,088 | 4,962,236 | 124,147 |
| mobile 390x844 | 3 | w=1200 | 4,924,610 | 130,082 | 4,962,236 | 168,141 |
...
| --- | --- | --- |
| 1 | 95 / 2765 ms | 99 / 2035 ms |
| 2 | 96 / 2720 ms | 96 / 2711 ms |
| 3 | 96 / 2712 ms | 99 / 2106 ms |

After-run 2 is an outlier: `next start` re-encodes a variant on an optimiser
cache miss and the request waits for sharp. On Vercel that encode is cached at
the edge, and it is not something the old `<img>` had an equivalent of.
...
crop is preserved by the same fact: `object-cover`'s crop is a function of the
box ratio against the source ratio, and both are unchanged.
...
`hero-compare-fuzz5--*.png` is `magick compare -fuzz 5%`, red on white: what is
left after ignoring deltas under 5%. It is thin edge outlines and nothing else,
no doubled or offset geometry, which is what a resample delta looks like and not
what a moved layout looks like. `hero-diffmap-8x--*.png` is
`|before - after|` amplified 8x on grey.

mobile-390x844@1x is the weak row, and it is honest to say so. A 390x844 CSS
viewport at DPR 1 does not ship on any phone (that viewport is an iPhone
12/13/14/15, always DPR 3), but the number is real: at 369 device px the browser
was supersampling a 3924px source, and a 384px WebP cannot match that on the
thin UI strokes inside the mockup. The >20% deltas are all in one place — the
"Maui, 1 ano e 7 meses" line, the three emoji outlines, the tab bar — and that
is 0.30% of the region. The photograph itself is indistinguishable at 3x zoom.
Every real-device path measures 38.3-40.8 dB.
